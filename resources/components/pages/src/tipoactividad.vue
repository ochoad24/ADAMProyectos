        ...
        <div class="col-lg-12">
            <b-card header="Tipo de Actividades" header-tag="h4" class="bg-primary-card">
                <div class="table-responsive">
                    <v-toolbar flat color="white">
                        ...
                        <v-dialog v-model="dialog" max-width="600px">
                            <template v-slot:activator="{ on }">
                                <v-btn color="green darken-1" dark class="mb-2" v-on="on">Nuevo tipo de actividad</v-btn>
                            </template>
                            <v-card>
                                <v-card-title>
                                    ...
                                    <v-container grid-list-md>
                                        <v-layout wrap>
                                            <v-flex xs12 sm12 md12>
                                                <v-text-field v-model="editedItem.nombre" label="Nombre del tipo de actividad">
                                                </v-text-field>
                                            </v-flex>
                                        </v-layout>
                                    </v-container>
                    ...
                    <v-data-table :headers="headers" :items="tipos" class="elevation-1" :search="search">
        ...
        methods: {
            validate() {
                ...
                if (!this.editedItem.nombre)
                    this.errorMsj.push('El nombre del tipo de actividad no puede estar vacio');
            ...
            deleteItem(item) {
                ...
                swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/TipoActividad/${item.id}/delete`).then(response => {
                            me.initialize();
                            swal.fire({
                            position: 'top-end',
                            type: 'success',
